...
class EditProfileViewViewModel: ObservableObject {
    @Published var name = User.shared.name
    @Published var height = User.shared.height.formatAsHeight()
    @Published var weight = String(User.shared.weight)
    @Published var gender = User.shared.gender
    @Published var birthday = User.shared.birthday
    ...
    func validate() -> Bool {
        ...
        guard let tenYearsAgo = Calendar.current.date(byAdding: .year, value: -10, to: birthday) else {
            return false
        }
